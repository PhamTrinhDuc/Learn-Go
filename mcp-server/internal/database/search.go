...
func (db *DB) VectorSearch(ctx context.Context, tenantID string, embedding []float32, limit int) ([]*VectorSearchResult, error) {
	...
	rows, err := tx.Query(ctx, queryString, vec, limit)
	if err != nil {
	...
	var documents []*VectorSearchResult
	...
	for rows.Next() {
		...
		documents = append(documents, &VectorSearchResult{
			Document: *doc,
			Score:    score,
		})
	}
...
func (db *DB) FullTextSearch(ctx context.Context, tenantID string, query string, limit int) ([]*FullTextSearchResult, error) {
	tx, err := db.BeginTx(ctx, tenantID)
	if err != nil {
		return nil, err
	}
	...
	queryString := `
		...
		FROM documents
		WHERE to_tsvector('simple', f_unaccent(title || ' ' || content)) @@ websearch_to_tsquery('simple', f_unaccent($1))
		LIMIT $2
	`

	rows, err := tx.Query(ctx, queryString, query, limit)
	if err != nil {
	...
	var documents []*FullTextSearchResult
	...
	for rows.Next() {
		...
		documents = append(documents, &FullTextSearchResult{
			Document: *doc,
		})
	}
	return documents, nil
