...
CREATE TABLE IF NOT EXISTS documents (
    ...
    embedding vector(1024),  -- Qwen3-embedding
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- Create dummy function if not exists for unaccent (needed for immutable wrapper)
...
-- Create full-text search index for BM25-like ranking (supporting unaccent)
DROP INDEX IF EXISTS idx_documents_fulltext;
CREATE INDEX idx_documents_fulltext ON documents USING gin(to_tsvector('simple', f_unaccent(title || ' ' || content)));
